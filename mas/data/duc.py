...
class DUC2004Writer:
    def __init__(self, args):
        self.source_dir = os.path.join(args.input_dir, 'docs/')
        self.target_dir = os.path.join(args.input_dir, 'eval/models/1/')
        self.output_dir = args.output_dir

    def write(self):
        source_reader = DUC2004SourceReader(self.source_dir)
        target_reader = DUC2004TargetReader(self.target_dir)

        if not os.path.exists(self.output_dir):
            os.makedirs(self.output_dir)

        with open(join(self.output_dir,'test.src'), 'w+') as sf, \
            open(join(self.output_dir,'test.1.tgt'), 'w+') as t1, \
            open(join(self.output_dir,'test.2.tgt'), 'w+') as t2, \
            open(join(self.output_dir,'test.3.tgt'), 'w+') as t3, \
            open(join(self.output_dir,'test.4.tgt'), 'w+') as t4:
            t_out = [t1, t2, t3, t4]
