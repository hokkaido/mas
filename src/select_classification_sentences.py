...
class SentenceSelector:
    ...
    def confine_docs(self, article, abstract):
        ...
        pos_sents = [s.text for s in ranked_sentences]
        neg_sents = []
        for orig_sent in article.sents:
            if orig_sent.text not in pos_sents:
                neg_sents.append(orig_sent.text)
        return '\n'.join(pos_sents) if len(pos_sents) > 0 else '', '\n'.join(neg_sents) if len(neg_sents) > 0 else ''
...
def main():
    ...
    i = 0
    with open(args.article_path) as article_file, open(args.abstract_path) as abstract_file, open(args.pos_output_path, 'w+') as pos_output_file, open(args.neg_output_path, 'w+') as neg_output_file:
        start = time.time()
        articles = []
        abstracts = []
        for x, y in zip(article_file, abstract_file):
            ...
            if i % args.batch_size == 0:
                ...
                for j in range(len(articles)):
                    pos, neg = selector.confine_docs(articles[j], abstracts[j])
                    print(pos, file=pos_output_file)
                    print(neg, file=neg_output_file)
                articles = []
                abstracts = []
                end = time.time()
